...
def _remove_topic_from_other_owners(combinations, topic, option):
    similar_topic_owners = [t for t in combinations.keys() if topic in type(t) and t!= topic]
    for sim_top_own in similar_topic_owners:
        combinations[sim_top_own]= [opt for opt in combinations[sim_top_own] if opt != option]

def _remove_other_topics_at_owner(combinations, topic, option):
    combinations[topic] = [opt for opt in combinations[topic] if opt not in type(option) or opt == option]
...
def _options_with_common_set_owner(combinations):
    for topic_owner, options in combinations.items():
        ...
        single_owner_options_type = [opt for opt, count in topic_counter.items() if count==1]
        single_owner_options = [opt for opt in options if any(opt in t for t in single_owner_options_type)]
        ...
        for topic_owner, options in combinations.items():
            ...
            for topic in topics:
                ...
                if len(opt_at_topic) == 1:
                    ...
                    print(f"single owner for {opt_at_topic} namely {topic_owner}. Removing option from other owners")
                    # We have a topic with a single option, hence only this owner can have the option
                    _remove_topic_from_other_owners(combinations, topic=topic_owner, option=opt_at_topic)
                    # We can mirror as well
                    _remove_topic_from_other_owners(combinations, topic=opt_at_topic, option=topic_owner)
                    # We can remove all other topics at the opposing owner
                    _remove_other_topics_at_owner(combinations, topic=opt_at_topic, option=topic_owner)
...
def solve(
    topics: list[StrEnum], facts: list[dict[StrEnum, list[StrEnum]]]
) -> Any:
    combinations = {}
    for topic in topics:
        for option in topic:
